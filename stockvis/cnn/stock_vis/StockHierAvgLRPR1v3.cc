...
public:
    unsigned w1, w2;
    Bigram(unsigned _w1, unsigned _w2): w1(_w1), w2(_w2) {}
    ...
    bool operator==(const Bigram &other) const
    { 
        return (w1 == other.w1 && w2 == other.w2);
    }
};
namespace std {
template <>
struct hash<Bigram>
{
    std::size_t operator()(const Bigram& key) const
    {
            using std::size_t;
            using std::hash;
            using std::string;
            return key.w1 * 10u + key.w2 * 100u;
    }
};
}
...
void computeLRPforRelu(cnn::real Rf, const vector<cnn::real>& w, const vector<cnn::real>& s, cnn::real b, vector<cnn::real>& Rs, cnn::real& Rb)
{
    //implement equation (2) to equation (6) of alex's note
    assert(w.size() == s.size());
    cnn::real sum = b;
    for(unsigned k = 0; k< s.size(); k++)
        sum += std::max(w[k] * s[k], 0.0f);//using relu here
    cnn::real append = epsilon * (sum > 0? 1: -1);
    sum += append;
    Rs.resize(s.size(), 0);
    for(unsigned k = 0; k< s.size(); k++)
        Rs[k] = Rf * std::max(w[k] * s[k], 0.0f) / sum;
    Rb = Rf * b / sum;
}
...
struct TitleEncoder {
    ...
    Expression buildLRPGraph(ComputationGraph&cg, const Example& example)
    {
        ...
        */
        ...
        vector<cnn::real> R_example_rep_relu(HIDDEN_DIM, 0);
        for(unsigned i = 0; i< INPUT_DIM; i++)
        {
            ...
            computeLRPforRelu(R_i_output[i], sw, example_rep_temp_values, example_rep_input_values[i], temp_R_example_rep, R_example_rep[i]);
            for(unsigned j = 0; j< HIDDEN_DIM; j++)
                R_example_rep_relu[j] += temp_R_example_rep[j];
        }
        ...
        for(unsigned i = 0; i< HIDDEN_DIM; i++)
        {
            ...
            computeLRP(R_example_rep_relu[i], sw, example_rep_input_values, param_tb_values[i], temp_R_example_rep);
            for(unsigned j = 0; j< INPUT_DIM; j++)
        ...
        cerr<<"[title_score]: "<< title_exps.size() << endl;
        for(unsigned i = 0; i< title_exps.size(); i++) //这个地方把标题的每一维度加起来可以实现得到每个标题的相关性
        {
            float title_sum = 0.0;
            for(unsigned j = 0; j< R_title_exps[i].size(); j++)//把title的每一维的分数加起来, 就是整个title的分数
                title_sum += R_title_exps[i][j];
            cerr<< i << "\t" << title_sum << endl;
        }
        cerr<< "===" << endl;
        ...
        unordered_map<Bigram, float> R_bigrams;
        ...
        for(size_t title_index = 0; title_index < example.words.size(); title_index++)
        {
            ...
            if(selected_words.size() > 1 )
            {
                ...
                Expression& current_col = title_cols[title_col_index]; //这个是tanh(bigram)的综合
                ...
                assert(current_col_values.cols() == i_words.size() -1u);
                ...
                //从R_current_col得到bigram的表示
                for(unsigned col_index = 0; col_index < selected_words.size()-1u; col_index++)
                {
                   Bigram bigram(selected_words[col_index], selected_words[col_index+1]); 
                   float bigram_sum = 0.0f;
                   for(unsigned row_index = 0; row_index< INPUT_DIM; row_index++)
                        bigram_sum += R_current_col[row_index][col_index];
                   R_bigrams[bigram] += bigram_sum;
                }
        ...
        //sort bigram
        vector<pair<Bigram, cnn::real> > R_sorted_bigrams;
        for(auto& item: R_bigrams)
            R_sorted_bigrams.push_back(make_pair(item.first, item.second));
        sort(R_sorted_bigrams.begin(), R_sorted_bigrams.end(), PairCmp<Bigram, cnn::real>());
        cerr<<"[bigram_score]: " << R_sorted_bigrams.size() << endl;
        for(auto& item: R_sorted_bigrams)
        {
            cerr<< word_dict.Convert(item.first.w1) << "\t" << word_dict.Convert(item.first.w2) << "\t" << item.second << endl;
        }
        cerr<< "===" << endl;
        ...
        cerr<<"[word_score]: " << R_sum_words.size() << endl;
