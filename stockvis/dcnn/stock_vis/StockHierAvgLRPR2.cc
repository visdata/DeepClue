...
void computeLRP_R2(cnn::real Rf, const vector<cnn::real>& w, const vector<cnn::real>& s, cnn::real b, vector<cnn::real>& Rs)
{
    //implement equation (2) to equation (6) of alex's note
    assert(w.size() == s.size());
    cnn::real sum_positive = std::max(b, 0.0);
    cnn::real sum_negative = std::min(b, 0.0);
    for(unsigned k = 0; k< s.size(); k++)
    {
        sum_positive += std::max(w[k] * s[k], 0.0);
        sum_negative += std::min(w[k] * s[k], 0.0);
    }
    Rs.resize(s.size(), 0);
    if(sum_positive == 0.0)
    ...
    for(unsigned k = 0; k< s.size(); k++)
        Rs[k] = Rf * ((1+beta) * std::max(w[k] * s[k], 0.0) / sum_positive - beta * std::min(w[k] *s[k], 0.0) / sum_negative);
}
/*
...
void computeLRPforSum_R2(float Rf, const vector<cnn::real>& s, vector<cnn::real>& Rs)
{
    ...
    for(unsigned k = 0; k< s.size(); k++)
    {
        sum_positive += std::max(s[k], 0.0);
        sum_negative += std::min(s[k], 0.0);
    }
    for(unsigned k = 0; k< s.size(); k++)
        Rs[k] = Rf  * ( (1+beta) *  std::max(s[k],0.0) / sum_positive - beta * std::min(s[k], 0.0)/sum_negative);
}
