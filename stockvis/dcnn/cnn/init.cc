...
#else
  kSCALAR_MINUSONE = (cnn::real*) cnn_mm_malloc(sizeof(cnn::real), 256);
  *kSCALAR_MINUSONE = -1;
  kSCALAR_ONE = (cnn::real*) cnn_mm_malloc(sizeof(cnn::real), 256);
  *kSCALAR_ONE = 1;
  kSCALAR_ZERO = (cnn::real*) cnn_mm_malloc(sizeof(cnn::real), 256);
  *kSCALAR_ZERO = 0;
#endif
  if (random_seed == 0) {
    random_device rd;
  ...
  fxs = new AlignedMemoryPool<ALIGN>(512UL*(1UL<<23));
  dEdfs = new AlignedMemoryPool<ALIGN>(512UL*(1UL<<23));
  cerr << "Done.\n";
}
