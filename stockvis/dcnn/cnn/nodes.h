...
struct CRFNode : public Node {
  explicit CRFNode(const std::initializer_list<VariableIndex>& a, const std::vector<int>& labels) : Node(a), gold_labels(labels){}
  std::string as_string(const std::vector<std::string>& arg_names) const override;
  Dim dim_forward(const std::vector<Dim>& xs) const override;
  void forward(const std::vector<const Tensor*>& xs, Tensor& fx) const override;
  void backward(const std::vector<const Tensor*>& xs,
  ...
  void uGradients(const std::vector<cnn::real>& grads, int token_index, Tensor& dEdxi, int s, int offset) const;
  void bGradients(const std::vector<cnn::real>& grads, int label_index, Tensor& dEdxi, int s, int offset) const ;
