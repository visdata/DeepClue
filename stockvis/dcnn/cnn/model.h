...
struct Parameters : public ParametersBase {
 ...
 private:
  Parameters() {}
  explicit Parameters(const Dim& d, cnn::real minmax, bool set_zero=false); // initialize with ~U(-minmax,+minmax)
...
struct LookupParameters : public ParametersBase {
 ...
 private:
  LookupParameters() {}
  LookupParameters(unsigned n, const Dim& d, bool set_zero=false);
  friend class boost::serialization::access;
...
class Model {
 public:
  ...
  Parameters* add_parameters(const Dim& d, cnn::real scale = 0.0f, bool set_zero=false);
  LookupParameters* add_lookup_parameters(unsigned n, const Dim& d, bool set_zero=false);
