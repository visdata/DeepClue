...
string CRFNode::as_string(const vector<string>& arg_names) const {
  ostringstream s;
  s << "pick( u" << arg_names[0] << ", b" <<  arg_names[1] << ')';
  return s.str();
}

Dim CRFNode::dim_forward(const vector<Dim>& xs) const {
  if(xs[0].ndims() != 2 || xs[1].ndims() != 2 || xs[0].rows() != xs[1].rows() || xs[0].rows() != xs[1].cols())
  {
    cerr << "Bad arguments in CRF: " << xs[0] <<"\t" << xs[1] << endl;
    throw std::invalid_argument("invalid arguments to CRF");
  }
  return Dim({1});
}
